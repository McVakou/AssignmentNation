...
public interface LanguageRepository extends JpaRepository<Language, Integer>, JpaSpecificationExecutor<Language> {
  ...
  @Query(value = "SELECT L.* FROM LANGUAGES L JOIN COUNTRY_LANGUAGES C ON L.LANGUAGE_ID = C.LANGUAGE_ID WHERE C.COUNTRY_ID = :countryId",
          nativeQuery = true)
  List<Language> findByCountryId(@Param("countryId") Long countryId);
}